document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`/api/duct_template?quotationId=${quotationId}`);
        if (!response.ok) {
...
function populateTable(data) {
    if (data.length > 0) {
        ...
        totalRow.innerHTML = `
            <td colspan="2" style="background-color: darkblue; color: white;"><b>Total</b></td>
...
async function fetchGoogleDocsContent() {
    try {
      ...
      const response = await fetch('/fetch-doc-supplyductsplit'); // Your API route
