document.addEventListener('DOMContentLoaded', async () => {
    ...
    searchBtn.addEventListener('click', () => {
        ...
        filteredQuotations = quotations.filter(quotation => quotation.agreement_id.toLowerCase().includes(searchTerm));
    ...
    async function fetchQuotations() {
        try {
            const response = await fetch('/api/agreement_villa');
            if (!response.ok) {
...
function generateQuotation(agreement_id, subcategory) {
    console.log(`Quotation ID: ${agreement_id}, Subcategory: ${subcategory}`); // Log the values
    
    let templatePage;
    
    switch (subcategory) {
        case 'duct':
            templatePage = 'duct_villa_agreement.html';
            break;
        case 'split':
            templatePage = 'split_villa_agreement.html';
            break;
        case 'duct&split':
            templatePage = 'duct_split_villa_agreement.html';
            break;
        case 'vrf':
    ...
    window.location.href = `${templatePage}?agreementId=${agreement_id}`;
}
