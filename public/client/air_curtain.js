...
function populateTable(data) {
    if (data.length > 0) {
        ...
        totalRow.innerHTML = `
            <td colspan="4" style="background-color: darkblue; color: white;"><b>Total</b></td>
            
            <td colspan="1" style="background-color: darkblue; color: white;"><b id="totalAmount">QAR ${totalSum.toLocaleString()}</b></td>
        `;
...
async function fetchGoogleDocsContent() {
    try {
      ...
      const response = await fetch('/fetch-doc-supplyaircurtain'); // Your API route
