document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function performSearch() {
        ...
        if (quotationId !== '') {
            ...
            query = `/api/supply_inst_data?quotationId=${encodeURIComponent(quotationId)}`;
        } else if (customerName !== '') {
            query = `/api/supply_inst_data?customerName=${encodeURIComponent(customerName)}`;
        } else if (mobileNumber !== '') {
            query = `/api/supply_inst_data?mobileNumber=${encodeURIComponent(mobileNumber)}`;
        } else {
    ...
    async function fetchAllRecords() {
        try {
            const response = await fetch('/api/supply_inst_data');
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
